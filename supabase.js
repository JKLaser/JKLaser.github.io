...
var SupabaseDB = (function () {
  ...
  function authRefreshSession() {
    ...
    return fetch(AUTH_URL + '/token?grant_type=refresh_token', {
      ...
      if (!newSession || typeof newSession.access_token !== 'string' || !newSession.access_token ||
          !newSession.user || !session.user || newSession.user.id !== session.user.id ||
          !Number.isFinite(newSession.expires_in) || newSession.expires_in <= 0) {
        throw new Error('刷新未返回有效会话，请重新登录');
      }
